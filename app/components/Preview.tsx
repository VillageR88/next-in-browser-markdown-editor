...
const Preview = ({ text, refProp }: { text: string; refProp: LegacyRef<HTMLDivElement> | undefined }) => {
  ...
  const parseText = (text: string) => {
    ...
    return text.split('\n').map((line, index) => {
      if (markMapArray.some((mark) => mark.includes(index - 1))) {
        ...
        return (
          <p
            className="font-robotoSlab text-[14px] leading-[24px] text-[#7C8187] transition-colors dark:text-[#C1C4CB]"
            key={index}
          >
  ...
  return (
    <div
      ref={refProp}
      className="no-scrollbar flex size-full flex-col gap-[12px] overflow-scroll break-words px-[23px] pb-[100px] pt-[30px] font-robotoMono text-[14px] leading-[40px] text-[#35393F] transition-colors dark:text-[#C1C4CB]"
    >
      {parseText(text)}
    </div>
  );
